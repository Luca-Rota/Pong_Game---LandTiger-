...
void GAME_Setup() {
	...
	for (i=0; i<320; i++){
		LCD_DrawLine(0, i, 4, i, Red);																							//disegno muro sx
		LCD_DrawLine(235, i, 239, i, Red);																					//disegno muro dx
	}
	
	for (i=278; i<288; i++)
		LCD_DrawLine(95, i, 144, i, Green);																					//disegno paddle
	
	for (i=32; i<42; i++)
		LCD_DrawLine(95, i, 144, i, Green);																					//disegno paddleIA	
			
	for(i=0;i<5;i++) 
		LCD_DrawLine( 230, 158+i, 234, 158+i, Green);   														//disegno pallina
	
	
	sprintf(str,"%d",score);
	GUI_Text(10, 150, (uint8_t *) str, White, Black);
	sprintf(str,"%d",scoreIA);
	GUI_Text2(228, 166, (uint8_t *) str, White, Black);
	GUI_Text(30, 100, (uint8_t *)  "Premi Key1 per giocare!", White, Black);		
...
void GAME_Restart(){
	...
	extern int x1p,x2p, x1pIA, x2pIA;
	extern int pause;
	
	for(i=0;i<5;i++) 
		LCD_DrawLine(x1, y2-i, x2, y2-i, Black);																		//cancello pallina
		
	for(i=278; i<288; i++){
		LCD_DrawLine(x1p, i, x2p, i, Black);																				//cancello paddle
		LCD_DrawLine(95, i, 144, i, Green);																					//disegno paddle
	}
	
	for(i=32; i<42; i++){
		LCD_DrawLine(x1pIA, i, x2pIA, i, Black);																		//cancello paddleIA
		LCD_DrawLine(95, i, 144, i, Green);																					//disegno paddleIA
	}
	...
	if (pause == 1){																															//se ero in pausa
		
		GUI_Text(95, 170, (uint8_t *) " Pausa ", Black, Black);											
		GUI_Text(15, 190, (uint8_t *) " Premi Int0 per riniziare.", Black, Black);	
	
	} else {																																			//se fine partita 
		
		if (score == 5){																														//se ho vinto
			
			GUI_Text2(165, 111, (uint8_t *)  "Hai Perso!", Black, Black);
			GUI_Text(85, 195, (uint8_t *)  "Hai Vinto!", Black, Black);
			
		} else {																																		//se ho perso
			
			GUI_Text2(165, 111, (uint8_t *)  "Hai Vinto!", Black, Black);
			GUI_Text(85, 195, (uint8_t *)  "Hai Perso!", Black, Black);
	...
	for (i=145; i<160; i++)
		LCD_DrawLine(6, i, 40, i, Black);																						//cancello punteggio
	
	sprintf(str,"%d",score);
	GUI_Text(8, 150, (uint8_t *) str, White, Black);
	
	for (i=145; i<160; i++)
		LCD_DrawLine(216, i, 230, i, Black);																				//cancello punteggioIA
